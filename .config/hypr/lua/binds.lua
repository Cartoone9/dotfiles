...
local mainMod = "ALT" -- Remapped for ALT as mainMod in the legacy config.
...
bind("SUPER + backspace", exec(scriptsDir .. "/LockScreen.sh"), "lock screen")
bind("SUPER + delete", exec(scriptsDir .. "/Logout.sh"), "close all apps, then exit Hyprland")
bind("SUPER + grave", exec(home .. "/.config/waybar/scripts/wifi-settings.sh"), "Wifi settings window")
...
bind("SUPER + H", exec(scriptsDir .. "/KeyBinds.sh"), "searchable keybinds list")
bind("SUPER + R", exec(scriptsDir .. "/Refresh.sh"), "refresh bar and menus")
bind("SUPER + E", exec(scriptsDir .. "/RofiEmoji.sh"), "emoji menu")
...
bind("SUPER + SHIFT_L", exec(scriptsDir .. "/ScreenShot.sh --area"), "screenshot (swappy)")
bind(mainMod .. " + SUPER + SHIFT_L", exec(scriptsDir .. "/ScreenRecord.sh"), "record selected area")
bind("CTRL + SUPER + SHIFT_L", exec(scriptsDir .. "/ScreenRecord.sh --full"), "record full screen")

-- SYSTEM
bind(mainMod .. " + SHIFT + Q", exec(scriptsDir .. "/KillActiveProcess.sh"), "Terminate active process")
bind("SUPER + escape", exec(scriptsDir .. "/Wlogout.sh"), "powermenu")
...
bind("SUPER + 1", exec(scriptsDir .. "/ChangeLayout.sh dwindle"), "layout dwindle")
bind("SUPER + 2", exec(scriptsDir .. "/ChangeLayout.sh master"), "layout master")
bind("SUPER + 3", exec(scriptsDir .. "/ChangeLayout.sh scrolling"), "layout scrolling")
...
bind("SUPER + comma", hl.dsp.layout("swapcol l"), "swap columns left")
bind("SUPER + period", hl.dsp.layout("swapcol r"), "swap columns right")
-- hyprctl keyword is rejected by the Lua config provider; hl.config via eval works
bind("SUPER + bracketright", exec("hyprctl eval \"hl.config({ scrolling = { direction = 'right' } })\""), "Horizontal scroll")
bind("SUPER + bracketleft", exec("hyprctl eval \"hl.config({ scrolling = { direction = 'down' } })\""), "Vertical scroll")
bind("SUPER + backslash", exec("bash -c '[[ $(hyprctl getoption scrolling:direction -j | jq -r \".str\") == \"right\" ]] && d=down || d=right; hyprctl eval \"hl.config({ scrolling = { direction = [[$d]] } })\"'"), "toggle scrolling V/H")
...
bind("SUPER + left", hl.dsp.window.swap({ direction = "l" }), "swap window left")
bind("SUPER + right", hl.dsp.window.swap({ direction = "r" }), "swap window right")
bind("SUPER + up", hl.dsp.window.swap({ direction = "u" }), "swap window up")
bind("SUPER + down", hl.dsp.window.swap({ direction = "d" }), "swap window down")
...
bind("SUPER + J", hl.dsp.window.cycle_next({ next = true }), "cycle next window")
bind("SUPER + K", hl.dsp.window.cycle_next({ prev = true }), "cycle previous window")
bind("SUPER + O", hl.dsp.layout("togglesplit"), "toggle split (dwindle)")
bind("SUPER + G", hl.dsp.group.toggle({}), "toggle group")
bind(mainMod .. " + CTRL + tab", hl.dsp.group.next({}), "change active in group")
bind("SUPER + tab", hl.dsp.group.next({}), "Change Group Forward")
bind("SUPER + SHIFT + tab", hl.dsp.group.prev({}), "Change Group Back")
bind("SUPER + CTRL + K", hl.dsp.window.move({ into_group = "l" }), "Move left into group")
bind("SUPER + CTRL + L", hl.dsp.window.move({ into_group = "r" }), "Move right into group")
bind("SUPER + CTRL + H", hl.dsp.window.move({ out_of_group = true }), "Move active out of group")
